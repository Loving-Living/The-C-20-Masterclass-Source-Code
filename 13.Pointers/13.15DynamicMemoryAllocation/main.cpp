...
//How we've used pointers so far
	...
	*p_number2 = 55; // Writing into junk address : BAD!
	...
	// 2) Initializing pointer to null
...
/* C uses the malloc() and calloc() function to allocate memory dynamically at run time 
   and uses a free() function to free dynamically allocated memory. C++ supports these 
   functions and also has two operators new and delete,that perform the task of 
   allocating and freeing the memory in a better and easier way.
   For initializing the memory for built-in data type:
   ...
   Allocate a block of memory: 
     ...
     Example:
	int *p = new int[10]
	>>>>Dynamically allocates memory for 10 integers continuously of type int and returns a pointer 
 	to the first element of the sequence, which is assigned top(a pointer). p[0] refers to the 
  	first element, p[1] refers to the second element, and so on. 
